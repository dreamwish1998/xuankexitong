...
def Student_Select_Lesson(request, number, semester):
    context = {}
    cursor = connection.cursor()
    ...
    return render(request, './student/open_lesson.html', context)

def Student_Select_Lesson_Search(request, number, semester, content):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select kh,km,gh,xm,mc,sksj from school_open_lesson tb1, school_lesson tb2,"
                   "school_teacher tb3,school_school tb4 where tb4.yxh=tb3.yxh_id and tb3.gh=tb1.gh_id and tb2.kh"
    ...
    return render(request, './student/open_lesson_search.html', context)
...
@require_http_methods(['POST'])
def Add_s_Optionlesson(request):
    response = {}
    stu_number = request.POST.get('stu_number')
    semester = request.POST.get('semester')
    lesson_number = request.POST.get('lesson_number')
...
def Edit_s_OptionLesson(request, number, semester):
    context = {}
    lesson_list = Option_lesson.objects.filter(xh=number, xq=semester)
    paginator = Paginator(lesson_list, 20)
    page_num = request.GET.get("page", 1)
    page_of_list = paginator.get_page(page_num)
    current_page_num = page_of_list.number
    ...
    context['page_of_list'] = page_of_list
    context['page_range'] = page_range
    return render(request, './student/edit_option_lesson.html', context)
...
def Delete_s_OptionLesson(request):
    if request.method == 'POST':
        student_number = request.POST.get('stu_number')
        semester = request.POST.get('semester')
        lesson_number = request.POST.get('lesson_id')
        ...
        lesson = Option_lesson.objects.filter(xh=Stu_table(xh=student_number), xq=semester,
                                              kh=Lesson(kh=lesson_number), gh=Teacher(gh=teacher_number)).first()
        lesson.delete()
        return redirect(reverse('Edit_s_OptionLesson', args=(student_number, semester)))
...
def S_Index(request):
    context = {}
    return render(request, './student/s_index.html', context)
...
def Get_Student_status(request):
    ...
    semester = request.POST.get('semester')
    number = request.POST.get('number')
    cursor.execute("select count(kh_id) from school_option_lesson where"
                   " xh_id="
                   "'"+number+"' and xq='"+semester+"';")
    les_number = list(cursor.fetchall())[0][0]

    cursor.execute("select sum(xf) from school_option_lesson tb1, school_lesson tb2 where"
                   " tb1.kh_id=tb2.kh and xh_id='"+number+"' and xq='"+semester+"';")
    class_points = list(cursor.fetchall())[0][0]

    cursor.execute("select count(kh_id),mc from school_option_lesson tb1, school_lesson tb2,school_school "
                   "tb3 where tb1.kh_id=tb2.kh and tb2.yxh_id=tb3.yxh and xh_id='"+number+"' and xq='"+semester+"' group by yxh_id;")
    les_distribution = cursor.fetchall()

    cursor.execute("select avg(zpcj),xq from school_option_lesson where xh_id='"+number+"' group by xq")
    semester_list = cursor.fetchall()

    cursor.execute("select xf,km from school_option_lesson tb1,school_lesson tb2 where tb1.kh_id=tb2.kh "
                   "and xh_id='"+number+"' and xq='"+semester+"';")
    points_list = cursor.fetchall()

    semesters = Option_lesson.objects.all().values('xq').distinct()
    response['les_distribute'] = list(les_distribution)
    response['les_number'] = les_number
    response['class_points'] = class_points
    response['semester_list'] = semester_list
    response['semesters'] = list(semesters)
    response['points'] = list(points_list)
    response['code'] = 0
    return JsonResponse(response)
...
def s_Lesson(request, number, semester):
    context = {}
    cursor = connection.cursor()
...
def s_Lesson_Search(request, number, semester, content):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select kh,km,gh,xm,sksj from school_option_lesson tb1,school_open_lesson tb2,school_lesson tb3,"
                   "school_teacher tb4,school_school tb5 where tb5.yxh=tb4.yxh_id and tb4.gh=tb1.gh_id and tb3.kh"
    ...
    context['page_of_list'] = page_of_list
    context['page_range'] = page_range
    return render(request, './student/lesson_search.html', context)
...
def s_Grade(request, number, semester):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select kh,km,gh,zpcj from school_option_lesson tb1,school_lesson tb2,school_teacher tb3 where tb1.kh_id=tb2.kh and tb1.gh_id=tb3.gh and"
                   " tb1.xh_id='"+number+"' and tb1.xq='"+semester+"';")
    lesson_list = cursor.fetchall()
    paginator = Paginator(list(lesson_list), 20)
    page_num = request.GET.get("page", 1)
    page_of_list = paginator.get_page(page_num)
    ...
    return render(request, './student/grade.html', context)

def s_Grade_Search(request, number, semester, content):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select kh,km,gh,zpcj from school_option_lesson tb1,school_lesson tb2,school_teacher tb3 where tb1.kh_id=tb2.kh and tb1.gh_id=tb3.gh and"
    ...
    return render(request, './student/grade_search.html', context)
...
def s_Get_Student_Note(request, xh, xq):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select tb1.gh_id, tb1.kh_id, created_time, content from school_option_lesson tb1, school_note_table tb2 "
                   "where tb1.gh_id=tb2.gh_id and tb1.kh_id=tb2.kh_id and tb1.xq=tb2.xq and tb1.xh_id='"+xh+"' and tb1.xq='"+xq+"';")
    ...
    context['page_of_list'] = page_of_list
    context['page_range'] = page_range
    return render(request, './student/student_note.html', context)

def s_Get_Student_Note_Search(request, xh, xq, content):
    context = {}
    cursor = connection.cursor()
    cursor.execute("select tb1.gh_id, tb1.kh_id, created_time, content from school_option_lesson tb1, school_note_table tb2 "
    ...
    context['page_of_list'] = page_of_list
    context['page_range'] = page_range
    return render(request, './student/student_note_search.html', context)
...
def s_Get_School_Note(request, xq):
    context = {}
